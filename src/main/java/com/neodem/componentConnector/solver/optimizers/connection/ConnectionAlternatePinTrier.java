...
public class ConnectionAlternatePinTrier implements ConnectionOptimizer {
	 ...
	 * 
	 * @see
	 * com.neodem.relayLocator.solver.optimizers.ConnectionOptimizer#optimize
	 * (com.neodem.relayLocator.model.Connection,
	 * com.neodem.relayLocator.model.sets.ComponentSet)
	 */
	...
	private boolean tryAltPins(Connection c, ComponentSet set, int initialTotalDistance, Collection<Pin> altPins,
			String id, boolean changed) {
		int best = initialTotalDistance;
		if (altPins.size() > 1) {

			// there are alternate pins.. lets try them all to find the best one
			...
			for (Pin altPin : altPins) {
				...
				if (setSize >= best) {
					...
					log.debug("on " + c + "," + id + " switching from " + current + " to " + altPin + " size = "
							+ setSize);
