...
/**
 * 
 * moves the 'to' side of the connection one component closer and tests to see if
...
public class ConnectionMover implements ConnectionOptimizer {
	...
	public int optimize(Connection c, ComponentSet set) {
		...
		if (!to.isMoveable()) {
			// we usually want to move the 'to' relay. If we can't but we can
			// move the 'from'
		...
		// check to see if we are not aligned already (can happen in diff rows)
		if (!ConnectionTools.inSameColumn(c)) {

			// Move 'mov' one closer horizontally
			// to 'fix' and see what happens. If we succeed, we return. If not
			// we revert and try horizontal shift

			// we are going to shift the 'mov' to the 'fix'. determine direction
			if (xFixed < xMovable) {
				// this means 'fix' is to the left of 'mov' so we shift 'mov'
				// one to
				// the left
				set.shiftOneLeft(mov);
			} else {
				// this means 'fix' is on the right of 'mov' so we shift 'mov'
				// one to
				// the right
